...
# File to store reminders and user data
...
class MemoryPingBot:
    def __init__(self):
        ...
        # Tips and tricks (shown instead of credits sometimes)
        self.tips = [
        ...
        self.message_count = 0  # Track messages to show credit occasionally
    ...
    def get_footer(self, show_credit=False):
        """Smart footer that shows tips or credit"""
        self.message_count += 1
        
        # Show credit only every 10th message or when explicitly requested
...
MESSAGES = {
    'en': {
        'welcome': "🔔 *Welcome to MemoryPing!*\n\nYour advanced personal reminder assistant!\n\n*Quick Commands:*\n/help - Full guide\n/quick - Quick reminders\n/list - View reminders\n/stats - Your statistics\n/settings - Preferences\n\n_Created by Achu Vijayakumar_ ✨",
        'help': "📖 *MemoryPing Complete Guide*\n\n*Basic Usage:*\n• Remind me to [task] at/in [time]\n• Send me [message] in [time]\n\n*Advanced Features:*\n🔄 Recurring: 'every day/week/month'\n🗂️ Categories: Add #work #health etc\n⚡ Priority: !high !medium !low\n📝 Notes: Add notes after --\n\n*Examples:*\n• Remind me to take medicine every day at 9am #health !high\n• Meeting at 3pm tomorrow #work -- Bring laptop\n\n_Created by Achu Vijayakumar_ ✨"
    }
}

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    lang = bot_instance.get_user_language(chat_id)
...
async def test_time_parsing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Debug command to test time parsing"""
    if not context.args:
        await update.message.reply_text(
            "🧪 *Time Parser Test*\n\n"
    ...
    if parsed_time:
        ...
        await update.message.reply_text(
            ...
            f"📥 Input: `{time_str}`\n"
            f"📤 Parsed: `{parsed_time.strftime('%I:%M %p, %b %d')}`\n"
            f"🕐 24h format: `{parsed_time.strftime('%H:%M')}`\n"
            f"⏳ In {time_until_mins:.0f} minutes",
            parse_mode='Markdown'
        )
    else:
        await update.message.reply_text(
            f"❌ *Parse Failed*\n\n"
            f"Input: `{time_str}`\n\n"
            f"Try: 3:30pm, 9pm, in 30 min, at 14:00",
            parse_mode='Markdown'
        )
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Append summary details and category breakdown, then send once
    message += f"\n\n⏰ Snoozed: {snoozed}\n"
    message += f"📈 Completion Rate: {completion_rate:.1f}%\n\n"
    if category_count:
        message += "*Active Reminders by Category:*\n"
        for category, count in sorted(category_count.items(), key=lambda x: x[1], reverse=True):
            emoji = bot_instance.categories.get(category, '📌')
...
def parse_time(time_str, current_time):
    ...
    # Parse "in/after X minutes/hours"
...
def extract_metadata(text):
    """Extract category, priority, notes, recurring, and shared users from text"""
    category = 'other'
    priority = 'medium'
    notes = ''
    recurring = None
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Use random fun confirmation message
    confirmation = bot_instance.get_random_confirmation()
    
    await update.message.reply_text(
        f"✅ *{confirmation}*\n\n"
        f"{priority_emoji} {task}\n"
        f"{cat_emoji} {category.capitalize()}\n"
        f"⏰ {remind_time.strftime('%I:%M %p, %b %d')}\n"
        f"⏳ In {time_msg.strip()}"
        f"{recurring_text}{shared_text}{notes_text}"
        f"{bot_instance.get_footer()}",
        parse_mode='Markdown'
    )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("quick_"):
        ...
        if template_key in bot_instance.templates:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("⏰ Custom", callback_data=f"template_{template_key}_custom")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    # Handle template time selection
        ...
        if minutes == "custom":
            await query.edit_message_text(
                "Please send me the time in format:\n`in 45 minutes` or `at 3:30pm`\n\n_Created by Achu Vijayakumar_ ✨",
                parse_mode='Markdown'
            )
            return
        ...
        template = bot_instance.templates[template_key]
        current_time = datetime.now()
        remind_time = current_time + timedelta(minutes=int(minutes))
        ...
        delay = (remind_time - current_time).total_seconds()
        # Fixed: Use correct job_queue reference
    ...
    # Handle snooze
        ...
        if new_time:
            ...
            # Reschedule with correct job_queue reference
            delay = (new_time - datetime.now()).total_seconds()
            if reminder_id in bot_instance.reminders:
                reminder = bot_instance.reminders[reminder_id]
                # Fixed: Use context.application.job_queue instead of context.job_queue
                if hasattr(context, 'application') and context.application.job_queue:
                    context.application.job_queue.run_once(
                        send_reminder, delay,
                        data={
            ...
            await query.edit_message_text(
                ...
                f"I'll ping you at {new_time.strftime('%I:%M %p')}",
                parse_mode='Markdown'
            )
        else:
            await query.edit_message_text(
    ...
    # Handle complete
        ...
        if bot_instance.complete_reminder(reminder_id):
            # Use random celebration message
